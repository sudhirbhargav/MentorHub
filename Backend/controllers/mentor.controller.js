...
const deleteAvailability = async (req, res) => {
    const { id } = req.params;
    try {
        const slot = await Availability.findOne({ _id: id, mentor: req.user._id });
...
const getSessionRequests = async (req, res) => {
    try {
        const requests = await Session.find({ mentor: req.user._id, status: 'pending' }).populate('mentee', 'name email');
        res.json(requests);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server error' });
...
const approveSessionRequest = async (req, res) => {
    ...
    try {
        const session = await Session.findOne({ _id: sessionId, mentor: req.user._id });
        if (!session) return res.status(404).json({ message: 'Session not found' });
        if (session.status !== 'pending') return res.status(400).json({ message: 'Session is not pending' });
...
const getMySessions = async (req, res) => {
    try {
        const sessions = await Session.find({ mentor: req.user._id, status: 'approved' }).populate('mentee', 'name email');
        res.json(sessions);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server error' });
...
const giveFeedback = async (req, res) => {
    ...
    try {
        ...
        if (session.status !== 'completed') {
            return res.status(400).json({ message: 'Feedback allowed only after session completion' });
        }
...
const getAnalytics = async (req, res) => {
    try {
        const sessions = await Session.find({ mentor: req.user._id, status: 'completed' });
        const sessionCount = sessions.length;
        const ratings = sessions.map(s => s.feedback.menteeRating).filter(Boolean);
...
module.exports = { setAvailability, getAvailability, deleteAvailability, getSessionRequests, approveSessionRequest, declineSessionRequest, getMySessions, giveFeedback, getAnalytics };